...
class UDPGame:
    def __init__(self, root):
        ...
        self.points = 3  # Initialize points here
        ...
        if self.is_auto_mode:
            ...
            if server_seq != expected_server_seq or server_ack != expected_server_ack:
                self.log_packet(
                    ...
                    is_error=True)
                self.update_points(-1)  # Deduct a point for incorrect response
            else:
                ...
                self.log_packet(
                    f"Server received packet: SEQ={self.server_seq}, ACK={self.server_ack}, Length={packet_length}")
                self.update_points(1)  # Add a point for correct response
